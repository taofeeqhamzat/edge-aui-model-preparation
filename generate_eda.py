    ...
    "cells": [
        ...
        },
        {
            ...
            "source": [
                ...
                "hvt_logs = glob.glob(os.path.join(hvt_dir, \"**\", \"*.csv\"), recursive=True)[:1]\n",
                "hvt_windows = []\n",
                "for log in hvt_logs:\n",
                ...
                "hmi_candidates = [f for f in glob.glob(os.path.join(hmi_dir, \"**\", \"*.csv\"), recursive=True) if \"raw_interactions\" in os.path.basename(f) or \"sequences\" in os.path.basename(f)]\n",
                "hmi_logs = hmi_candidates[:2] if hmi_candidates else glob.glob(os.path.join(hmi_dir, \"**\", \"*.csv\"), recursive=True)[:2]\n",
                "hmi_windows = []\n",
                "for log in hmi_logs:\n",
                "    hmi_windows.extend(process_hmi_sequences(log, max_records=500))\n",
                ...
                "ap_candidates = [f for f in glob.glob(os.path.join(ap_dir, \"**\", \"*.json\"), recursive=True) if \"study\" in f or \"data\" in f]\n",
                "ap_logs = ap_candidates[:5] if ap_candidates else glob.glob(os.path.join(ap_dir, \"**\", \"*.json\"), recursive=True)[:5]\n",
                "ap_windows = []\n",
                "for log in ap_logs:\n",
